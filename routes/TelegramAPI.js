...
const TELEGRAM_COURSE_SUGGESTION_AI_TIMEOUT_MS = Number(
  process.env.TELEGRAM_COURSE_SUGGESTION_AI_TIMEOUT_MS || 90 * 1000,
);
...
const TELEGRAM_GEMINI_MODEL = process.env.GEMINI_MODEL || "gemini-2.5-flash";
...
const TELEGRAM_COURSE_FIELD_HINTS = {
  course_name: ["course", "subject", "module", "مقرر", "مادة", "اسم المقرر"],
  course_component: [
    "lecture",
    "lab",
    "practical",
...
const pickLatestUsableTelegramGroupField = (values, groupReference) =>
  (Array.isArray(values) ? values : [])
    ...
    .find(
      (value) =>
        Boolean(value) && normalizeGroupReference(value) !== groupReference,
    ) || "";
...
const buildTelegramEntityCandidates = (entity, dialog) => {
  const username = normalizeGroupReference(
    entity?.username || dialog?.entity?.username,
  );
  const id = String(entity?.id || dialog?.id || "").trim();
  const title = String(entity?.title || dialog?.title || "")
    .trim()
    .toLowerCase();
...
const resolveTelegramGroupEntity = async (client, reference) => {
  ...
  const matchingDialog = dialogs.find((dialog) => {
    ...
    return candidates.some(
      (candidate) =>
        String(candidate || "").toLowerCase() === normalizedReferenceLower,
    );
  });
  ...
  if (
    isTelegramUserEntity(fallbackEntity) ||
    !isTelegramGroupEntity(fallbackEntity)
  ) {
    const error = new Error(
...
const enrichTelegramPdfPayload = async ({ message, payload }) => {
  if (!payload?.attachmentIsPdf || !payload?.id) {
    return payload;
  }

  // Store only Telegram file ID, access hash, and file name for frontend download
  const document = message?.media?.document || message?.document || null;
  return {
    ...payload,
    telegramFileId: document?.id || null,
    telegramAccessHash: document?.accessHash || null,
    telegramFileName: payload.attachmentFileName || null,
  };
};
...
const buildNormalizedCoursePayload = (payload = {}) => {
  const courseExams = (
    Array.isArray(payload?.course_exams) ? payload.course_exams : []
  )
  ...
  return {
    ...
    course_grade: String(
      primaryExam.course_grade || payload?.course_grade || "",
    ).trim(),
    course_fullGrade: String(
      primaryExam.course_fullGrade || payload?.course_fullGrade || "",
    ).trim(),
    ...
    exam_type:
      String(primaryExam.exam_type || payload?.exam_type || "-").trim() || "-",
    exam_date:
      String(primaryExam.exam_date || payload?.exam_date || "-").trim() || "-",
    exam_time:
      String(primaryExam.exam_time || payload?.exam_time || "-").trim() || "-",
  };
};
...
const hasArabicCharacters = (value) =>
  /[\u0600-\u06ff]/.test(String(value || ""));
    ...
    courseArabic:
      suggestionStage === "name_prediction"
        ...
        : {
            ...
            course_name:
              normalizedArabicName || normalizedPrimaryPayload.course_name,
          },
    courseEnglish:
      suggestionStage === "name_prediction"
...
const scoreMessageForCourseSuggestion = (message) => {
  ...
  const hasTitleLikeSurface = [fileNameText, messageText].some(
    (value) =>
      Boolean(String(value || "").trim()) && !isLikelyCourseMetaOnlyText(value),
  );
  ...
  (Array.isArray(messages) ? messages : [])
    ...
    .filter(
      (entry) =>
        ...
        entry.snippet &&
        (entry.score > 0 ||
          (entry.attachmentFileName && entry.hasTitleLikeSurface)),
    )
    .sort(
...
const buildInstructorSuggestionContextEntries = (messages = []) =>
  (Array.isArray(messages) ? messages : [])
    .map((message) => {
      const scoreResult = scoreMessageForCourseSuggestion(message);
      const rawContent = [
      ...
      const hasInstructorSignal =
        normalizedContent.includes("doctor") ||
        normalizedContent.includes("dr ") ||
        normalizedContent.includes(" dr") ||
        normalizedContent.includes("professor") ||
        normalizedContent.includes("instructor") ||
        normalizedContent.includes("lecturer") ||
        normalizedContent.includes("دكتور") ||
        normalizedContent.includes("الدكتور") ||
        normalizedContent.includes("الأستاذ") ||
        normalizedContent.includes("استاذ") ||
        normalizedContent.includes("المدرس");
      ...
      return {
        ...
        attachmentFileName: String(message?.attachmentFileName || "").trim(),
        matchedKeys: scoreResult.matchedKeys,
        score: Number(scoreResult.score || 0) + (hasInstructorSignal ? 8 : 0),
        snippet: buildMessageSnippet(message),
      };
    })
    .filter(
      (entry) =>
        entry.messageId &&
        entry.snippet &&
        (entry.score > 0 || Boolean(entry.attachmentFileName)),
    )
    .sort(
      (firstEntry, secondEntry) =>
        Number(secondEntry.score || 0) - Number(firstEntry.score || 0) ||
...
const TELEGRAM_COURSE_META_ONLY_WORD_PATTERN =
  /^(?:مثبت(?:ة)?|مثبت|الفصل|الترم|السمستر|semester|term|year|level|first|second|third|fourth|fifth|sixth|الاول|الأول|الثاني|الثالث|الرابع|الخامس|السادس|اول|ثاني|ثالث|رابع|خامس|سادس|السنة|الأولى|الثانية|الثالثة|الرابعة|الخامسة|السادسة|1st|2nd|3rd|4th|5th|6th|1|2|3|4|5|6|عملي|نظري|lecture|lab|practical|theory)$/i;
...
const hasMeaningfulCourseTitleStructure = (value = "") => {
  ...
  if (
    (plainValue.match(/[\u0621-\u064A]/g) || []).length > 0 &&
    longWords.length === 0
  ) {
    return false;
  }
...
const normalizeTelegramCourseSuggestionMatchedKeys = (matchedKeys = []) => {
  ...
  return ["course_name", ...uniqueKeys.filter((key) => key !== "course_name")];
};
...
const isLikelyCourseMetaOnlyText = (value = "") => {
  ...
  const hasSemesterOrYearSignal =
    /الفصل|الترم|السمستر|semester|term|year|السنة|level|اول|أول|ثاني|ثالث|رابع|خامس|سادس|first|second|third|fourth|fifth|sixth|1st|2nd|3rd|4th|5th|6th|1|2|3|4|5|6/i.test(
      plainValue,
    );
  const nonMetaWords = words.filter(
    (word) =>
      word.length >= 3 && !TELEGRAM_COURSE_META_ONLY_WORD_PATTERN.test(word),
  );
  ...
  return (Array.isArray(rawSuggestions) ? rawSuggestions : [])
    .map((suggestion, index) => {
      ...
      const sourceMessageIds = (
        Array.isArray(suggestion?.sourceMessageIds)
  ...
  suggestions: (Array.isArray(suggestions) ? suggestions : [])
    .filter(
      (suggestion) =>
        ...
        normalizeTelegramCourseSuggestionMatchedKeys(suggestion?.matchedKeys)
          .length > 0,
    )
    .map((suggestion) => ({
      suggestionKey: String(suggestion?.suggestionKey || "").trim(),
      duplicateKey: String(suggestion?.duplicateKey || "").trim(),
      suggestionStage: String(
        suggestion?.suggestionStage || "name_prediction",
      ).trim(),
      confidence: Math.max(
        0,
        Math.min(100, Number(suggestion?.confidence || 0)),
...
const removeTelegramCourseSuggestionFromSavedRecord = ({
  ...
  savedRecords.forEach((savedRecord) => {
    ...
    if (
      originGroupReference &&
      Number(savedRecord?.sourceMessageId || 0) !==
        Number(originSourceMessageId || 0)
    ) {
      return;
    }
...
const buildTelegramCourseSuggestionFeedbackContext = (
  ...
  (Array.isArray(feedbackEntries) ? feedbackEntries : [])
    .filter((entry) => {
      if (
        isTelegramCourseSuggestionAllGroupsScope(
          groupReference,
          sourceMessageId,
        )
      ) {
...
const buildPersistedTelegramLectureSuggestionsRecord = ({
  groupReference = "",
  groupTitle = "",
  suggestions = [],
  analyzedMessagesCount = 0,
  ...
  analyzedMessagesCount: Math.max(0, Number(analyzedMessagesCount || 0)),
  suggestions: (Array.isArray(suggestions) ? suggestions : [])
    .filter((lecture) => Boolean(String(lecture?.lectureName || "").trim()))
    .map((lecture) => ({
      suggestionKey:
        String(lecture?.suggestionKey || "").trim() ||
        buildLectureSuggestionDuplicateKey(lecture),
      duplicateKey: buildLectureSuggestionDuplicateKey(lecture),
      lectureName: String(lecture?.lectureName || "").trim(),
      lecturePayload: lecture?.lecturePayload || {},
      confidence: Math.max(0, Math.min(100, Number(lecture?.confidence || 0))),
...
const upsertTelegramLectureSuggestionsRecord = ({
  user,
  groupReference = "",
  groupTitle = "",
  suggestions = [],
  ...
  if (!Array.isArray(user.schoolPlanner.telegramLectureSuggestions)) {
    user.schoolPlanner.telegramLectureSuggestions = [];
  }

  const record = buildPersistedTelegramLectureSuggestionsRecord({
    groupReference,
    groupTitle,
    suggestions,
    analyzedMessagesCount,
...
const getSavedTelegramLectureSuggestionsRecord = (user, groupReference = "") =>
...
const saveTelegramLectureSuggestionFeedback = ({
  user,
  groupReference = "",
  groupTitle = "",
  decision = "",
  ...
  if (!Array.isArray(user.schoolPlanner.telegramLectureSuggestionFeedback)) {
    user.schoolPlanner.telegramLectureSuggestionFeedback = [];
  }

  user.schoolPlanner.telegramLectureSuggestionFeedback.unshift({
    groupReference: String(groupReference || "").trim(),
    groupTitle: String(groupTitle || "").trim(),
    decision: String(decision || "").trim(),
    savedAt: new Date(),
...
const removeTelegramLectureSuggestionFeedbackByScope = ({
  ...
  user.schoolPlanner.telegramLectureSuggestionFeedback = (
    Array.isArray(user?.schoolPlanner?.telegramLectureSuggestionFeedback)
      ? user.schoolPlanner.telegramLectureSuggestionFeedback
      : []
  ).filter(
    (entry) =>
...
const buildPersistedTelegramInstructorSuggestionsRecord = ({
  groupReference = "",
  groupTitle = "",
  suggestions = [],
  analyzedMessagesCount = 0,
  ...
  suggestions: (Array.isArray(suggestions) ? suggestions : [])
    ...
    .map((entry) => ({
      ...
      duplicateKey: buildInstructorSuggestionDuplicateKey(entry),
...
const upsertTelegramInstructorSuggestionsRecord = ({
  user,
  groupReference = "",
  groupTitle = "",
  suggestions = [],
  ...
  if (!Array.isArray(user.schoolPlanner.telegramInstructorSuggestions)) {
    user.schoolPlanner.telegramInstructorSuggestions = [];
  }

  const record = buildPersistedTelegramInstructorSuggestionsRecord({
    groupReference,
    groupTitle,
    suggestions,
    analyzedMessagesCount,
...
const saveTelegramInstructorSuggestionFeedback = ({
  user,
  groupReference = "",
  groupTitle = "",
  decision = "",
  ...
  if (!Array.isArray(user.schoolPlanner.telegramInstructorSuggestionFeedback)) {
    user.schoolPlanner.telegramInstructorSuggestionFeedback = [];
  }

  user.schoolPlanner.telegramInstructorSuggestionFeedback.unshift({
    groupReference: String(groupReference || "").trim(),
    groupTitle: String(groupTitle || "").trim(),
    decision: String(decision || "").trim(),
    savedAt: new Date(),
...
const removeTelegramInstructorSuggestionFeedbackByScope = ({
  ...
  user.schoolPlanner.telegramInstructorSuggestionFeedback = (
    Array.isArray(user?.schoolPlanner?.telegramInstructorSuggestionFeedback)
      ? user.schoolPlanner.telegramInstructorSuggestionFeedback
      : []
  ).filter(
    (entry) =>
...
const parseQueryDateValue = (value, mode = "start") => {
  ...
  const normalizedValue =
    hasExplicitTime || mode === "start" ? rawValue : `${rawValue}T23:59:59.999`;
...
const scoreMessageAgainstQuery = (messageText, queryText) => {
  ...
  return {
    matched:
      matchedTerms.length > 0 || normalizedMessage.includes(normalizedQuery),
    score,
    matchedTerms,
  };
};
...
const listJoinedTelegramGroupsForUser = async (user) => {
  ...
  try {
    ...
    storedSummariesByReference.forEach((storedSummary, groupReference) => {
      ...
      dialogEntriesByReference.set(groupReference, {
        groupReference,
        title: storedSummary.title || storedSummary.username || groupReference,
    ...
    try {
      ...
      if (
        ...
        applyTelegramSyncResult(user, {
          ...
          message:
            "Telegram history sync was skipped because a sync ran very recently.",
        });
        await user.save();
      ...
      let newestStoredMessageDateMs = user.telegramIntegration
        ?.lastStoredMessageDate
      ...
      if (lastStoredMessageId > 0) {
        for await (const message of client.iterMessages(entity, {
          ...
          if (messageDateMs) {
            newestMessageDateSeenMs = Math.max(
              newestMessageDateSeenMs,
              messageDateMs,
            );
          ...
          newestStoredMessageId = Math.max(
...
TelegramRouter.get("/groups", checkAuth, async (req, res, next) => {
  try {
    ...
    try {
      groups = await listJoinedTelegramGroupsForUser(user);
    } catch (error) {
  ...
  async (req, res, next) => {
    try {
      ...
      return res.status(200).json({
        ...
        dateMs:
          typeof messageRecord.dateMs === "number"
            ? messageRecord.dateMs
            : null,
...
TelegramRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const viewMode = String(req.query.view || "messages")
        .trim()
        .toLowerCase();
...
TelegramRouter.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const requestGroupReference = normalizeGroupReference(
      ...
      if (contextEntries.length === 0) {
        ...
        setTelegramCourseSuggestionStatus(userId, {
          ...
          phase: "completed",
          message:
            "No stored message content was available for course suggestions.",
        });
        return res.status(200).json({
          suggestions: [],
          analyzedMessagesCount: 0,
      ...
      priorSuggestedCourses.forEach((suggestion) => {
        if (
          ...
          !existingCourseIndex.has(suggestion.duplicateKey)
        ) {
  ...
  "suggestions": [
    {
      ...
      const attemptSizes = [aiContextEntries.length, 4, 2, 1].filter(
      ...
      for (const attemptSize of attemptSizes) {
        ...
        try {
          ...
          if (
            !isTimeout ||
            attemptSize === attemptSizes[attemptSizes.length - 1]
          ) {
            setTelegramCourseSuggestionStatus(userId, {
              active: false,
              phase: "failed",
              message:
                errorMessage ||
                "AI request failed while generating course name predictions.",
            });
    ...
    try {
      ...
      return res.status(200).json({
        suggestions: Array.isArray(savedRecord?.suggestions)
          ? savedRecord.suggestions.filter(
              (suggestion) =>
                hasMeaningfulCourseTitleStructure(
                  suggestion?.coursePayload?.course_name || "",
                ) &&
        ...
        sourceAttachmentFileName: String(
          savedRecord?.sourceAttachmentFileName || "",
        ).trim(),
        allGroups,
      });
    } catch (error) {
      ...
      const suggestions = (
        ...
        .filter((entry) => {
          ...
          return (
            ...
            normalizeTelegramCourseSuggestionMatchedKeys(entry?.matchedKeys)
              .length > 0
          );
        })
        .map((entry, index) => ({
...
TelegramRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const openAiClient = getOpenAIClient();
      const user = await UserModel.findById(req.authentication.userId).select(
        "schoolPlanner.courses schoolPlanner.telegramLectureSuggestions info.username info.program info.university info.studyYear info.term info.aiProvider",
      );
  ...
  "lectures": [
    {
      ...
      const lectures = (Array.isArray(parsed?.lectures) ? parsed.lectures : [])
        .map((lecture) => ({
          ...
          lecturePayload: {
            ...
            lecture_instructor: String(
              lecture?.lecturePayload?.lecture_instructor || "",
            ).trim(),
            lecture_writer: String(
              lecture?.lecturePayload?.lecture_writer || "",
            ).trim(),
            lecture_date: String(
              lecture?.lecturePayload?.lecture_date || "",
            ).trim(),
            lecture_length: String(
              lecture?.lecturePayload?.lecture_length || "",
            ).trim(),
            lecture_progress: String(
              lecture?.lecturePayload?.lecture_progress || "",
            ).trim(),
            lecture_partOfPlan:
              lecture?.lecturePayload?.lecture_partOfPlan !== false,
            lecture_hidden: Boolean(lecture?.lecturePayload?.lecture_hidden),
          },
          confidence: Math.max(
            0,
      ...
      const lectureScopeGroupReference =
        requestedGroupReferences.length === 1
          ? String(requestedGroupReferences[0] || "").trim()
          : TELEGRAM_COURSE_SUGGESTION_ALL_GROUPS_SCOPE;

      upsertTelegramLectureSuggestionsRecord({
        user,
        groupReference: lectureScopeGroupReference,
        groupTitle:
          requestedGroupReferences.length === 1
            ? String(
                (
...
TelegramRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const openAiClient = getOpenAIClient();
      const user = await UserModel.findById(req.authentication.userId).select(
        "schoolPlanner.telegramInstructorSuggestions info.username info.program info.university info.studyYear info.term info.aiProvider",
      );
      ...
      if (contextEntries.length === 0) {
        return res.status(404).json({
          message: "No stored Telegram evidence matched instructor names.",
        });
      }
      ...
      if (!aiProvider) {
        return res.status(503).json({
          message:
            "AI instructor predictions are unavailable because no AI provider key is configured.",
          code: "TELEGRAM_AI_PROVIDER_UNAVAILABLE",
        });
      }
      ...
      const aiInstructions = `You predict instructor names for the user's university from stored Telegram messages.
Return JSON only.
Use the user's academic profile as a strict filter, especially university, program, and study year.
Return only real person names that are likely professors, doctors, lecturers, or instructors from the user's university context.
Do not invent names.
Do not return course names, department names, or generic titles without a real person name.
Avoid duplicates and prefer fewer high-confidence names.
${buildTelegramAiLanguageInstruction(outputLanguage)}
Return this JSON shape:
{
  "instructors": [
    {
      ...
      try {
        response = await runWithTimeout(
          ...
          TELEGRAM_AI_REQUEST_TIMEOUT_MS,
          "AI request timed out while generating instructor predictions.",
        );
      } catch (error) {
        const errorMessage =
          error?.message || "Unable to complete the AI request.";
      ...
      const instructorScopeGroupReference =
        requestedGroupReferences.length === 1
          ? String(requestedGroupReferences[0] || "").trim()
          : TELEGRAM_COURSE_SUGGESTION_ALL_GROUPS_SCOPE;

      upsertTelegramInstructorSuggestionsRecord({
        user,
        groupReference: instructorScopeGroupReference,
        groupTitle:
          requestedGroupReferences.length === 1
            ? String(instructorScopeGroupReference).trim()
            : "All stored groups",
...
TelegramRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await UserModel.findById(req.authentication.userId).select(
        "schoolPlanner.telegramLectureSuggestionFeedback",
      );
...
TelegramRouter.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      removeTelegramLectureSuggestionFeedbackByScope({ user, groupReference });
      await user.save();

      return res.status(200).json({
        message: "Rejected lecture suggestions cleared.",
        allGroups,
      });
    } catch (error) {
      next(error);
...
TelegramRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await UserModel.findById(req.authentication.userId).select(
        "schoolPlanner.telegramInstructorSuggestionFeedback",
      );
...
TelegramRouter.post(
  "/ai/instructor-suggestions/feedback",
  checkAuth,
  async (req, res, next) => {
    try {
      const allGroups = Boolean(req.body?.allGroups);
  ...
  try {
    ...
    return res.status(200).json({
      group: {
        id: groupSnapshot?.groupId || null,
        title:
          groupSnapshot?.groupTitle ||
          userConfig.groupReference ||
          "Telegram Group",
        username: groupSnapshot?.groupUsername || null,
        pageUrl: userConfig.pageUrl,
      },
...
TelegramRouter.post("/send-note", checkAuth, async (req, res, next) => {
  ...
  try {
    ...
    const targetMode = requestedTargetMode === "group" ? "group" : "saved";
    ...
    return res.status(200).json({
      ...
      targetMode,
      target: targetMode === "group" ? targetReference : "me",
    });
  } catch (error) {
    next(error);
  } finally {
